...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{7.2cm}X}
        ...
        \hline
        \texttt{src/pipelines/}\newline\texttt{jokes.py}                & Загрузка сырых корпусов, очистка, удаление дублей и сохранение корпуса шуток               \\
        \texttt{src/pipelines/}\newline\texttt{embeddings.py}           & Построение плотных векторных представлений шуток                                           \\
        \texttt{src/pipelines/}\newline\texttt{keywords.py}             & Извлечение ключевых слов с использованием сходства и MMR                                   \\
        \texttt{src/pipelines/}\newline\texttt{references.py}           & Построение многореференсных запросов через векторный поиск                                 \\
        \texttt{src/pipelines/}\newline\texttt{candidates.py}           & Генерация кандидатных ответов разных моделей                                               \\
        \texttt{src/pipelines/}\newline\texttt{evaluation.py}           & Попарная автоматическая оценка и построение таблиц лидеров                                 \\
        \texttt{src/training/}\newline\texttt{reference\_likelihood.py} & Вычисление правдоподобия эталонных ответов через принудительное учительское декодирование  \\
        \texttt{src/training/}\newline\texttt{mrvf\_trainer.py}         & Обучение LoRA-адаптера с MRVF-целью                                                        \\
        \texttt{scripts/}\newline\texttt{train\_mrvf.py}                & Основной скрипт запуска обучения                                                           \\
        \hline
    \end{tabularx}
    \endgroup
\end{table}
