...
def _download_short_jokes() -> Path:
    destination_dir = DATA_DIR / "short-jokes"
    destination_dir.mkdir(parents=True, exist_ok=True)

    permalink = (
        "https://github.com/amoudgl/short-jokes-dataset/blob/79c59bf8392929da3c560a3fa207be44e15b65db/shortjokes.csv"
    )
    filename = _get_filename_from_permalink(permalink)
    url = _get_raw_github_url(permalink)
    _download_file(url, destination_dir / filename)
    return destination_dir


def _download_r_jokes() -> Path:
    ...
    for permalink in permalinks:
        filename = _get_filename_from_permalink(permalink)
        url = _get_raw_github_url(permalink)
        gz_path = _download_file(url, destination_dir / filename)
        _unzip_gz_file(gz_path)
    return destination_dir
...
def _preprocess_short_jokes(destination_dir: Path) -> Path:
...
def _preprocess_r_jokes(destination_dir: Path) -> Path:
    output_path = destination_dir / "data.parquet"
...
def build_jokes_dataset() -> Path:
    short_jokes_dir = _download_short_jokes()
    short_jokes_path = _preprocess_short_jokes(short_jokes_dir)

    r_jokes_dir = _download_r_jokes()
    r_jokes_path = _preprocess_r_jokes(r_jokes_dir)
