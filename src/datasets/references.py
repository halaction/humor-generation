...
def build_references_dataset() -> Path:
    ...
    keywords = load_dataset("parquet", data_dir=str(keywords_dir), split="train")

    pipeline = ReferencesPipeline()
    output_dir = asyncio.run(pipeline.run(keywords=keywords, embeddings=embeddings, jokes=jokes, resume=True))
    logger.info(
        "build.done",
...
def publish_references_dataset(
    repo_id: str = settings.HF_DATASET_REPO_ID,
    config_name: str = config.references.hf_config_name,
    split: str = "train",
    private: bool = False,
) -> tuple[str, str]:
    output_dir = DATA_DIR / config.references.hf_config_name
    if not output_dir.exists():
        output_dir = build_references_dataset()
